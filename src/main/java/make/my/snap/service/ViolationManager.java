...
public class ViolationManager {
    ...
    public ViolationManager(SnapAI plugin) {
        this.plugin = plugin;
        ...
        this.mlConsoleFormat = plugin.getConfig().getString(
                ...
                "[SnapAI] ML flag: %player% prob=%prob% VL=%vl%"
        );
        this.checkConsoleFormat = plugin.getConfig().getString(
                "alerts.console.check-format",
                "[SnapAI] Check flag: %player% check=%check% VL=%vl%"
        );
        this.webhookService = plugin.getDiscordWebhookService();
    ...
    private void executePunishment(PlayerEntity entity, String mode) {
        ...
        if ("INSTANT".equals(mode)) {
            ...
            commandToExecute = "snapai punish " + entity.getName();
        }
    ...
    private void sendLegacyAlert(PlayerEntity entity, String checkName, double currentVl) {
        ...
        String finalMessage = ChatColor.translateAlternateColorCodes('&',
                message.replace("%player%", entity.getName())
                        ...
                        .replace("%vl%", vlString));
        String permission = plugin.getConfig().getString("alerts.permission", "snapai.alerts");
        ...
        if (webhookService != null) {
            webhookService.sendLegacyAlert(entity, checkName, vlString);
        }
    }
    ...
    private void sendAlert(PlayerEntity entity, double probability, double currentVl) {
        ...
        if (webhookService != null) {
            webhookService.sendMlAlert(entity, formattedProb, vlString);
        }
    }
